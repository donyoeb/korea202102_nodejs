...
class Hero extends GameObject{
    constructor(container, src, width, height, x, y, velX, velY){
        ...
        this.jump =false; //주인공이 점프상태를 판단할수있는 변수 
        //이변수는 상속과 상관없이 내꺼!!!
    }
    //히어로는 움직임지 있다! 따라서 메서드 정의가 요구된다
    ...
    tick(){
        ...
        this.velY=this.velY+this.g; //중력을 표현하기 위해 가속도로 처리!!

        this.y=this.y+this.velY;
        this.x = this.x +this.velX;
    
        //현재 화면에 존재하는 모든 벽들들을 대상으로, 주인공의 발바닥과 닿았는지 체크
        for(var i=0;i<blockArray.length;i++){

            var onBlock = collisionCheck(this.img , blockArray[i].img);
            
            //onblock이 true라면 벽돌에 발이 닿은것
            if(onBlock && this.jump==false){
                //점프를 누르면 yelY값을 0으로 묶지않도록 조건추가
                 
                this.velY= 0;//속도없애기 
                
                //위치를 벽돌에 고정.  (벽돌의 y값보다 자신의 키만큼 위로올리기)
                this.y = blockArray[i].y - this.height; 
            }

            //주인공이 점프한 이후, 다시 하강하는 순간을 포착하여 
            //벽돌위에 서있을수있는 핵심 변수인 this.jump를 다시 false로 돌려놓자
            if(this.velY > 0) this.jump = false;
